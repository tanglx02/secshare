...
| 系统设置（**页签式**） | 分「站点 / 邮件 / 支付」三个页签：<br>· **站点**：站点信息、链接结构（Permalink）、SEO 增强（canonical/结构化数据/站长验证/分享图）、联系方式、功能开关、变现开关、法律文本<br>· **邮件**：SMTP 配置（密码加密存储）、连通性自检、发送测试邮件、注册邮箱验证开关、验证码查询、同 IP 注册上限<br>· **支付**：支付宝沙箱·生产切换、APPID、应用私钥与支付宝公钥（加密存储）、扫码或电脑网站支付、连通性自检、支付日志 |
...
4. **签约能力**：
   - 扫码支付（当面付）→ 签约「当面付」
   - 电脑网站支付 → 签约「电脑网站支付」
5. **填入后台**：后台 →「系统设置 → 支付」页签 → 填 `APPID`、`应用私钥`、`支付宝公钥`（⚠️ 不是应用公钥），选择支付方式，保存
...
后台 →「系统设置 → 邮件」页签可开启 **注册必须邮箱验证码**，从源头挡住脚本批量注册。

### 配置步骤

1. 后台 →「系统设置 → 邮件」→ 填入 SMTP 信息：
